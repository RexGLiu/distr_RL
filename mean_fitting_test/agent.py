...
from model import C51, DQN, mean_var_DQN, mean_var_DQN2, mean_var_skew_DQN
...
class Rainbow_mean_var_51(Rainbow):
  def learn(self, mem):
    ...
    loss = F.mse_loss(Q_a, Q_target) + F.mse_loss(var_a, var_target)
    self.online_net.zero_grad()
    (weights * loss).mean().backward()  # Backpropagate importance-weighted minibatch loss
    clip_grad_norm_(self.online_net.parameters(), self.norm_clip)  # Clip gradients by L2 norm
    self.optimiser.step()
...
class Rainbow_mean_var_DQN():
  def __init__(self, args, env):
    assert args.multi_step == 1, "TD variance learning only supported for single step target"
  ...
  def learn(self, mem):
    ...
    loss = F.mse_loss(Qs_a, Q_target) + self.weight * F.mse_loss(var_Qs_a, var_target)
    self.online_net.zero_grad()
    (weights * loss).mean().backward()  # Backpropagate importance-weighted minibatch loss
    clip_grad_norm_(self.online_net.parameters(), self.norm_clip)  # Clip gradients by L2 norm
    self.optimiser.step()
...
class Rainbow_mean_var_skew_DQN():
  ...
  def learn(self, mem):
    ...
    loss = F.mse_loss(Qs_a, Q_target) + self.weight * F.mse_loss(var_Qs_a, var_target) + self.weight * F.mse_loss(skew_Qs_a, skew_target)
    self.online_net.zero_grad()
    (weights * loss).mean().backward()  # Backpropagate importance-weighted minibatch loss
    clip_grad_norm_(self.online_net.parameters(), self.norm_clip)  # Clip gradients by L2 norm
    self.optimiser.step()
